"""Contract for the DeepSeek V4 C4 indexer query projection on MI300X."""
...
ADAPTER_ID = "c4_indexer_q_projection"
...
def _representative_cases():
    ...
    for m in PREFILL_M:
        ...
        cases.append(
            CaseSpec(
                f"prefill_c4_indexer_q_projection_m{m}_context65536",
                {
                    "phase": "prefill",
                    "m": m,
                    "k": K,
    ...
    for m in DECODE_M:
        ...
        cases.append(
            CaseSpec(
                f"decode_c4_indexer_q_projection_m{m}_context65536",
                {
                    "phase": "decode",
                    "m": m,
                    "k": K,
...
class DeepSeekV4AiterC4IndexerQProjectionSpec:
    operator_id = "deepseek_v4_aiter_c4_indexer_q_projection"

    def cases(self):
        return (
            CaseSpec(
                "prefill_smoke_m16_context1024",
                {
                    "phase": "prefill",
                    "m": 16,
                    "k": K,
    ...
    def make_inputs(self, case, context):
        ...
        if generator is None:
            raise RuntimeError("C4 indexer query projection requires a GPU generator")
        x = torch.randn(
        ...
        return OracleStateComparator(
            numeric_paths=(
                NumericPath(
                    ...
                    # The strict gate above is candidate-vs-current-SGLang-reference.
                    # Keep the existing block-FP8-vs-FP32 semantic-oracle diagnostic
                    # separate; otherwise the quantized reference itself cannot pass.
                    oracle_atol=1.0,
                    oracle_rtol=0.05,
                ),
            ),
            ...
            name="sglang_aiter_c4_indexer_q_projection_oracle",
        )
...
SPEC = DeepSeekV4AiterC4IndexerQProjectionSpec()
