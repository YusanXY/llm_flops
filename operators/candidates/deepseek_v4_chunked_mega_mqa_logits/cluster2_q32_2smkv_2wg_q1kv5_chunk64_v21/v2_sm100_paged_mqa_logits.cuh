...
struct SM100PagedMQALogitsScheduler : SM100IndicesStorage<kIsVarlen> {
    ...
    // Each rank contributes sixteen query rows.  Eight N256 cta_group::2 passes
    // consume them as two rows per rank while reusing the same M256 KV tile.
    ...
    uint32_t cur_request_q_token_start; // request base for Q32 weights/output
