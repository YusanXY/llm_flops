...
V4 keeps the spill-free shared-weight reduction and the same total shared
memory budget, but changes the software pipeline from two Q plus four KV
stages to one Q plus five KV stages.  NCU on V3 reports long-scoreboard as the
largest non-selected PM-sampling state, while KV traffic dominates Q traffic;
this variant tests whether one extra prefetched KV split is worth exposing the
next Q-tile load.  It is promoted only if formal performance improves.
