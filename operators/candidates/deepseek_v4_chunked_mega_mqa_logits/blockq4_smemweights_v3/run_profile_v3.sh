...
CANDIDATE="$ROOT/operators/candidates/deepseek_v4_chunked_mega_mqa_logits/blockq4_smemweights_v3"
ART="$ROOT/results/deepseek_v4_chunked_mega_mqa_logits/blockq4_smemweights_v3_single_request_20260731"
...
flock -w7200 /tmp/mega-mqa-gpu3.lock bash -lc "
    ...
    if [[ '$MODE' == ncu ]]; then
      exec /usr/local/cuda/bin/ncu \
        ...
        --replay-mode kernel \
        --force-overwrite \
        --export '$ART/ncu/m${M}_blockq4_smemweights_v3_set_full' \
    ...
    if [[ '$MODE' == nsys ]]; then
      exec nsys profile \
        ...
        --output='$ART/nsys/m${M}_blockq4_smemweights_v3' \
        '$PY' '$CANDIDATE/profile_candidate.py' --m '$M' --capture
    fi
