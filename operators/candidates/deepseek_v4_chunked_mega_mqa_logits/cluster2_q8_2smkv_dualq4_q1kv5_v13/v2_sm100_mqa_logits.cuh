...
CUTLASS_DEVICE void sm100_mqa_logits_core_impl(const uint32_t logits_stride,
    ...
    static constexpr bool kDebugSkipMma = false;
    static constexpr bool kDebugSkipMath = false;
    ...
    DG_STATIC_ASSERT(kNumKVPeers == 2, "V13 requires a two-CTA cluster");
    DG_STATIC_ASSERT(kNumMathWarpGroups == 1 and kNumMathThreads == 128,
                     "V13 uses one M128 TMEM consumer group per CTA");
    DG_STATIC_ASSERT(SPLIT_KV == UMMA_M and kLocalSplitKV == 128,
                     "V13 requires a logical M256 split with M128 per CTA");
    ...
    if (warp_idx == kSpecWarpStart) {
        ...
        cutlass::arch::warpgroup_reg_dealloc<kNumSpecializedRegisters>();
        DG_STATIC_ASSERT(not kIsFP4, "V13 currently targets the formal FP8 path");
        ...
        while (scheduler.next_q_block(q_block_idx, kv_base, num_kv_splits)) {
            ...
            for (uint32_t kv_split_idx = 0; kv_split_idx < num_kv_splits; ++ kv_split_idx) {
                ...
                if (cluster_rank == 1) {
                    ...
                    if constexpr (not kDebugSkipMma) {
                ...
                if constexpr (kDebugSkipMath) {
                    if (cute::elect_one_sync()) {
                        smem.full_tmem_barriers[0].wait(0);
                        smem.full_tmem_barriers[1].wait(0);
                    }
                }
                __syncwarp();
            }
        ...
        DG_STATIC_ASSERT(kNumMathWarpGroups == 1, "one consumer warpgroup per CTA");
        ...
        while (scheduler.next_q_block(q_block_idx, kv_base, num_kv_splits)) {
            ...
            for (uint32_t kv_split_idx = 0; kv_split_idx < num_kv_splits; ++ kv_split_idx) {
                ...
                // Both UMMA passes have committed to TMEM and this CTA has
                // consumed its local scale, so its M128 KV half is reusable.
                smem.empty_kv_barriers[kv_stage_idx].arrive();
            }
