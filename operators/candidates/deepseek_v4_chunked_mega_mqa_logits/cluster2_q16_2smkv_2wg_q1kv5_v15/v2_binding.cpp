...
torch::Tensor v2_forward(
    ...
    TORCH_CHECK(q.dim() == 4, "q must have [B,16,H,D] layout");
    TORCH_CHECK(q.size(1) == 16 && q.size(2) == 64 && q.size(3) == 128,
                "V15 supports one two-CTA cluster per next_n=16 group, H=64, D=128");
    ...
    TORCH_CHECK(weights.dim() == 2 &&
                weights.size(0) == q.size(0) * 16 &&
                weights.size(1) == 64,
                "weights must have [B*16,64] layout");
    TORCH_CHECK(context_lens.dim() == 2 &&
                context_lens.size(0) == q.size(0) &&
                context_lens.size(1) == 16,
                "context_lens must have [B,16] layout");
