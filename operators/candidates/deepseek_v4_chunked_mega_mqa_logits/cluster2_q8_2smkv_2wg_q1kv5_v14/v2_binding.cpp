...
torch::Tensor v2_forward(
    ...
    TORCH_CHECK(q.size(1) == 8 && q.size(2) == 64 && q.size(3) == 128,
                "V10 supports one two-CTA cluster per next_n=8 group, H=64, D=128");
    ...
    constexpr int kQueriesPerCluster = 8;

    const auto batch = static_cast<int>(q.size(0));
    const auto total_q = batch * kQueriesPerCluster;
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("forward", &v2_forward, "cluster2 Q8 cta_group::2 KV SM100 paged MQA logits");
}
