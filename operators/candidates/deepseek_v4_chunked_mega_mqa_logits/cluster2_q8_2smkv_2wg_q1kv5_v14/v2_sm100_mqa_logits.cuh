...
CUTLASS_DEVICE void sm100_mqa_logits_core_impl(const uint32_t logits_stride,
    ...
    // BLOCK_Q=4 and H=64 consume 256 TMEM columns per accumulator
    // stage.  Two stages fill the architectural 512-column allocation
    // exactly and still cover the two math warp groups.
    ...
    // Two N256 instructions reuse that same KV tile to cover the cluster's
    // eight queries (four queries per instruction).
    ...
    DG_STATIC_ASSERT(kNumKVPeers == 2, "V14 requires a two-CTA cluster");
    DG_STATIC_ASSERT(kNumMathWarpGroups == 2 and kNumMathThreads == 256,
                     "V14 assigns one M128 TMEM consumer group to each Q pair");
    DG_STATIC_ASSERT(SPLIT_KV == UMMA_M and kLocalSplitKV == 128,
                     "V14 requires a logical M256 split with M128 per CTA");
    ...
    // Each math warpgroup owns one Q pair and one TMEM stage.  Splitting the
    // two epilogues removes V13's simultaneous two-Q-pair live state and
    // restores the proven V4 register budget.
    ...
    if (warp_idx == kSpecWarpStart) {
        ...
        cutlass::arch::warpgroup_reg_dealloc<kNumSpecializedRegisters>();
        DG_STATIC_ASSERT(not kIsFP4, "V14 currently targets the formal FP8 path");
            ...
            for (uint32_t kv_split_idx = 0; kv_split_idx < num_kv_splits; ++ kv_split_idx) {
                ...
                if (cluster_rank == 1) {
                    ...
                    // lane for both MMA and commit.
                    {
                        ...
                        #pragma unroll
                        for (uint32_t q_pair = 0; q_pair < 2; ++q_pair) {
        ...
        while (scheduler.next_q_block(q_block_idx, kv_base, num_kv_splits)) {
            ...
            for (uint32_t kv_split_idx = 0; kv_split_idx < num_kv_splits; ++ kv_split_idx) {
                ...
                CUTE_TIE_DECL(kv_pipeline.advance(), kv_stage_idx, kv_phase);
                smem.full_kv_barriers[kv_stage_idx].wait(kv_phase);
                const reduce_dtype_t scale_kv = static_cast<reduce_dtype_t>(
                    ptx::ld_shared(smem.smem_sf_kv[kv_stage_idx] + math_thread_idx));

                // Warpgroup 0 consumes q-pair 0 / TMEM stage 0 while
                // warpgroup 1 consumes q-pair 1 / TMEM stage 1.  Advancing
                // by two keeps that ownership stable across KV iterations.
                constexpr uint32_t kTmemStridePerIteration = kNumMathWarpGroups;
                CUTE_TIE_DECL(tmem_pipeline.advance(kTmemStridePerIteration),
                              tmem_stage_idx, tmem_phase);
                smem.full_tmem_barriers[tmem_stage_idx].wait(tmem_phase);
                ptx::tcgen05_after_thread_sync();
                const uint32_t q_pair = math_warpgroup_idx;

                #pragma unroll
                for (uint32_t i = 0; i < BLOCK_Q; ++i) {
                        // cta_group::2 concatenates rank-0 and rank-1 B halves.
                        // q_pair 0 covers local rows [0,2), q_pair 1 rows [2,4).
                        const uint32_t q_in_cluster =
                            (i < 2) ? (q_pair * 2 + i)
                                    : (BLOCK_Q + q_pair * 2 + i - 2);
...
// V10 paged scheduler pairs two Q4 CTAs.  The CTAs walk identical KV chunks,
// select disjoint halves of each Q8 group and share one multicast KV stream.
...
void sm100_paged_mqa_logits(const uint32_t num_q_tokens_total,
                            ...
                            const __grid_constant__ cute::TmaDescriptor tensor_map_sf_kv,
                            const __grid_constant__ cute::TmaDescriptor tensor_map_weights) {
    static constexpr uint32_t BLOCK_Q = 4;
    ...
    sm100_mqa_logits_core_impl<kIsFP4, kNumHeads, kHeadDim, false, BLOCK_Q, SPLIT_KV,
                               ...
                               kNumSpecializedThreads, kNumMathThreads, logits_dtype_t,
                               reduce_dtype_t, decltype(make_scheduler), kNumMathWarpGroups, 2>(
        logits_stride, logits,
        tensor_map_q, tensor_map_sf_q, tensor_map_kv, tensor_map_sf_kv, tensor_map_weights,
        make_scheduler);
}
