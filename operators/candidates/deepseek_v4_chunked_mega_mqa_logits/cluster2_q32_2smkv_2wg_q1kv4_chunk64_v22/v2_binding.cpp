...
torch::Tensor v2_forward(
    ...
    TORCH_CHECK(q.dim() == 4, "q must have [B,32,H,D] layout");
    TORCH_CHECK(q.size(1) == 32 && q.size(2) == 64 && q.size(3) == 128,
                "V22 supports one two-CTA cluster per next_n=32 group, H=64, D=128");
    ...
    TORCH_CHECK(weights.dim() == 2 &&
                weights.size(0) == q.size(0) * 32 &&
                weights.size(1) == 64,
                "weights must have [B*32,64] layout");
    TORCH_CHECK(context_lens.dim() == 2 &&
                context_lens.size(0) == q.size(0) &&
                context_lens.size(1) == 32,
                "context_lens must have [B,32] layout");
    ...
    constexpr int kQueriesPerCluster = 32;
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("forward", &v2_forward, "cluster2 Q32 cta_group::2 KV SM100 paged MQA logits");
}
