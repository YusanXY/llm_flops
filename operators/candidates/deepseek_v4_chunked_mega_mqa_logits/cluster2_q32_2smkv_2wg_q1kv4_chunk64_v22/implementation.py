"""V22: two-CTA cta_group::2 Q32 with pipelined Q-pair epilogues.
...
The device implementation groups sixteen adjacent query positions per CTA solely
...
* eight N256 MMA passes reuse the same KV tile and cover all thirty-two queries;
* two 256-column accumulator stages exactly fill the 512-column TMEM budget;
* one Q and four KV shared-memory stages exploit the longer Q32 compute window
  while reducing dynamic shared memory by one 16.5-KB KV stage;
...
_BUILD = _HERE / ".build_cluster2_q32_2smkv_2wg_q1kv4_chunk64_v22"
_BUILD.mkdir(exist_ok=True)

os.environ.setdefault("TORCH_CUDA_ARCH_LIST", "10.0a")

_ext = load(
    name="mega_chunked_mqa_cluster2_q32_2smkv_2wg_q1kv4_chunk64_v22",
    ...
    with_cuda=True,
    verbose=os.environ.get("MEGA_V22_VERBOSE_BUILD", "0") == "1",
)
...
def operator(
    ...
    if m % QUERIES_PER_CLUSTER:
        raise ValueError("cluster2 Q32 requires a query count divisible by thirty-two")
