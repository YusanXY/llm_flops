...
CUTLASS_DEVICE void sm100_mqa_logits_core_impl(const uint32_t logits_stride,
    ...
    // allocation and alternate across the eight Q-pair passes.
    ...
    // Eight N256 instructions reuse that same KV tile to cover the cluster's
    // thirty-two queries (two local query rows from each rank per instruction).
    ...
    DG_STATIC_ASSERT(kNumKVPeers == 2, "V22 requires a two-CTA cluster");
    DG_STATIC_ASSERT(kNumMathWarpGroups == 2 and kNumMathThreads == 256,
                     "V22 alternates Q pairs across two consumer groups");
    DG_STATIC_ASSERT(SPLIT_KV == UMMA_M and kLocalSplitKV == 128,
                     "V22 requires a logical M256 split with M128 per CTA");
    DG_STATIC_ASSERT(BLOCK_Q == 16 and kNumQPairs == 8,
                     "V22 requires Q16 per CTA and eight N256 passes");
    ...
    if (warp_idx == kSpecWarpStart) {
        ...
        cutlass::arch::warpgroup_reg_dealloc<kNumSpecializedRegisters>();
        DG_STATIC_ASSERT(not kIsFP4, "V22 currently targets the formal FP8 path");
...
// V22 pairs two Q16 CTAs.  The CTAs walk identical KV chunks and contribute
// disjoint Q halves to eight cta_group::2 N256 passes.
...
void sm100_paged_mqa_logits(const uint32_t num_q_tokens_total,
                            ...
                            const __grid_constant__ cute::TmaDescriptor tensor_map_sf_kv,
                            const __grid_constant__ cute::TmaDescriptor tensor_map_weights) {
    static constexpr uint32_t BLOCK_Q = 16;
