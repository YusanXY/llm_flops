...
CUTLASS_DEVICE void sm100_mqa_logits_core_impl(const uint32_t logits_stride,
    ...
    static constexpr uint32_t kQueriesPerMMA = 4;
    ...
    DG_STATIC_ASSERT(kNumKVPeers == 2, "V20 requires a two-CTA cluster");
    DG_STATIC_ASSERT(kNumMathWarpGroups == 2 and kNumMathThreads == 256,
                     "V20 alternates four Q pairs across two consumer groups");
    DG_STATIC_ASSERT(SPLIT_KV == UMMA_M and kLocalSplitKV == 128,
                     "V20 requires a logical M256 split with M128 per CTA");
    DG_STATIC_ASSERT(BLOCK_Q == 8 and kNumQPairs == 4,
                     "V20 requires Q8 per CTA and four N256 passes");
    ...
    if (warp_idx == kSpecWarpStart + 1 and cute::elect_one_sync()) {
        ...
        for (uint32_t i = 0; i < kNumKVStages; ++ i) {
            // Each cta_group::2 TMA producer contributes one arrival to the
            // leader transaction barrier.  Match the two-CTA arrival count
            // used by the official SM100 multicast pipeline.
    ...
    if (warp_idx == kSpecWarpStart) {
            ...
            for (uint32_t kv_split_idx = 0; kv_split_idx < num_kv_splits; ++ kv_split_idx) {
                if constexpr (decltype(scheduler)::kIsPaged) {
                    ...
                    if (cute::elect_one_sync()) {
                        ...
                        // Both CTAs execute one cta_group::2 TMA operation
                        // with rank-dependent page coordinates.  Hardware
                        // deposits one disjoint M128 half in each CTA and
                        // merges both completion byte counts into rank 0's
                        // transaction barrier.
                        #pragma unroll
                        for (uint32_t page_idx = 0; page_idx < kLocalPages; ++ page_idx) {
                    ...
                    tma::copy<kNumQKBytesPerToken, kLocalSplitKV, 0>(
                        ...
                        smem.smem_kv[kv_stage_idx], 0, kv_tma_offset, 2);
                    tma::copy<kLocalSplitKV, 1, 0>(&tensor_map_sf_kv, &smem.full_kv_barriers[kv_stage_idx],
                                              smem.smem_sf_kv[kv_stage_idx], kv_tma_offset, 0, 2);
                    if (cluster_rank == 0) {
                        smem.full_kv_barriers[kv_stage_idx].arrive_and_expect_tx(
                            2 * (SMEM_KV_SIZE_PER_STAGE + SMEM_SF_KV_SIZE_PER_STAGE));
                    } else {
                        smem.full_kv_barriers[kv_stage_idx].arrive(0u);
                    }
                }
                __syncwarp();
            }
        ...
        cutlass::arch::warpgroup_reg_dealloc<kNumSpecializedRegisters>();
        DG_STATIC_ASSERT(not kIsFP4, "V20 currently targets the formal FP8 path");
        ...
        while (scheduler.next_q_block(q_block_idx, kv_base, num_kv_splits)) {
            ...
            for (uint32_t kv_split_idx = 0; kv_split_idx < num_kv_splits; ++ kv_split_idx) {
                ...
                #pragma unroll
                for (uint32_t q_pair = math_warpgroup_idx;
                     q_pair < kNumQPairs;
                     q_pair += kNumMathWarpGroups) {
...
// V20 pairs two Q8 CTAs.  The CTAs walk identical KV chunks and contribute
...
void sm100_paged_mqa_logits(const uint32_t num_q_tokens_total,
                            ...
                            const __grid_constant__ cute::TmaDescriptor tensor_map_sf_kv,
                            const __grid_constant__ cute::TmaDescriptor tensor_map_weights) {
    static constexpr uint32_t BLOCK_Q = 8;
