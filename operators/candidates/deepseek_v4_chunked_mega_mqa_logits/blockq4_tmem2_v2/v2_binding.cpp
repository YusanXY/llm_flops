...
torch::Tensor v2_forward(
    ...
    TORCH_CHECK(q.dim() == 4, "q must have [B,4,H,D] layout");
    TORCH_CHECK(q.size(1) == 4 && q.size(2) == 64 && q.size(3) == 128,
                "V2 supports next_n=4, H=64, D=128");
    ...
    TORCH_CHECK(weights.dim() == 2 &&
                weights.size(0) == q.size(0) * 4 &&
                weights.size(1) == 64,
                "weights must have [B*4,64] layout");
    TORCH_CHECK(context_lens.dim() == 2 &&
                context_lens.size(0) == q.size(0) &&
                context_lens.size(1) == 4,
                "context_lens must have [B,4] layout");
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("forward", &v2_forward, "BLOCK_Q4/TMEM2 SM100 paged MQA logits");
}
