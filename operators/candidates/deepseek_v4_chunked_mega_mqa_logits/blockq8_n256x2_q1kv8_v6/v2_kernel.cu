...
void launch_blockq8_n256x2_tmem1(
    ...
    using Storage = deep_gemm::layout::MQALogitsSharedStorage<
        false, 64, 128, 8, 128, 1, 8, 1, float>;
    constexpr int kSmemBytes = static_cast<int>(sizeof(Storage));
    constexpr int kThreads = 128 + 128;
    ...
    deep_gemm::sm100_paged_mqa_logits<
        ...
        128, 128,
        float, float,
        1><<<grid_size, kThreads, kSmemBytes, stream>>>(
    ...
    if (status != cudaSuccess) {
        throw std::runtime_error(
            std::string("BLOCK_Q8/N256x2/TMEM1 kernel launch failed: ") +
            cudaGetErrorString(status));
    }
}
