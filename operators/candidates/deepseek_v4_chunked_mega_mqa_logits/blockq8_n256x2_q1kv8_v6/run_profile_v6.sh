...
CANDIDATE="$ROOT/operators/candidates/deepseek_v4_chunked_mega_mqa_logits/blockq8_n256x2_q1kv8_v6"
ART="$ROOT/results/deepseek_v4_chunked_mega_mqa_logits/blockq8_n256x2_q1kv8_v6_single_request_20260731"
...
flock -w7200 /tmp/mega-mqa-gpu3.lock bash -lc "
    ...
    if [[ '$MODE' == ncu ]]; then
      exec /usr/local/cuda/bin/ncu \
        ...
        --replay-mode kernel \
        --force-overwrite \
        --export '$ART/ncu/m${M}_blockq8_n256x2_q1kv8_v6_set_full' \
    ...
    if [[ '$MODE' == nsys ]]; then
      exec nsys profile \
        ...
        --output='$ART/nsys/m${M}_blockq8_n256x2_q1kv8_v6' \
        '$PY' '$CANDIDATE/profile_candidate.py' --m '$M' --capture
    fi
