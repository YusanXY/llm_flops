...
CUTLASS_DEVICE void sm100_mqa_logits_core_impl(const uint32_t logits_stride,
    ...
    // BLOCK_Q=8 and H=64 consume all 512 TMEM columns in one accumulator
    // stage.  The logical N=512 result is emitted as two explicit N=256
    // TCGen05 instructions because N=256 is the native legal instruction
    // tile on this path.
    static constexpr uint32_t kNumTmemStages = 1;
    ...
    static constexpr uint32_t UMMA_N_INSTR = 256;
    ...
    DG_STATIC_ASSERT(UMMA_N == 2 * UMMA_N_INSTR, "V6 requires an N=512 logical tile");
    ...
    if (warp_idx == kSpecWarpStart) {
        ...
        if (cute::elect_one_sync()) {
            ...
            while (scheduler.next_q_block(q_block_idx, kv_base, num_kv_splits)) {
                ...
                DG_STATIC_ASSERT(BLOCK_Q * kNumHeads == 512, "V6 expects two 256-row Q TMA boxes");
                #pragma unroll
                for (uint32_t q_half = 0; q_half < 2; ++ q_half) {
                    tma::copy<kNumQKBytesPerToken, 256, 0>(
                        &tensor_map_q, &smem.full_q_barriers[q_stage_idx],
                        smem.smem_q[q_stage_idx] + q_half * 256 * kNumQKBytesPerToken,
                        0, q_token_base * kNumHeads + q_half * 256);
                }
                if constexpr (kIsFP4)
                    ...
                    for (uint32_t i = 0; i < kNumMathWarpGroups; ++ i) {
                        ...
                        if constexpr (kIsFP4) {
                            ...
                            auto instr_desc = cute::UMMA::make_instr_desc<cutlass::float_e4m3_t, cutlass::float_e4m3_t, float,
                                                                            UMMA_M, UMMA_N_INSTR, cute::UMMA::Major::K, cute::UMMA::Major::K>();
                            ...
                            for (uint32_t q_half = 0; q_half < 2; ++ q_half) {
                                ...
                                for (uint32_t k = 0; k < kHeadDim / UMMA_K; ++ k) {
                                    ...
                                    auto b_desc = mma::sm100::make_umma_desc<cute::UMMA::Major::K, 0, kHeadDim, kHeadDim>(
                                        smem.smem_q[q_stage_idx], q_half * UMMA_N_INSTR, k * UMMA_K);
                                    ptx::SM100_MMA_F8F6F4_SS::fma(
                                        a_desc, b_desc, tmem_addr + q_half * UMMA_N_INSTR,
                                        k, runtime_instr_desc);
                                }
                            }
                        }
            ...
            const auto process_q_block = [&](auto num_valid_tokens_t) {
                ...
                for (uint32_t kv_split_idx = 0; kv_split_idx < num_kv_splits; ++ kv_split_idx) {
                    ...
                    for (uint32_t i = 0; i < kNumValidTokens; ++ i) {
                        // BLOCK_Q8 needs 8 x 64 FP32 weights.  Keeping that
                        // complete tile live in registers forces ptxas to
                        // spill it to local memory.  All lanes in a warp read
                        // the same weight address, so explicit shared-memory
                        // broadcast is both exact and cheaper than the
                        // implicit local/L2 reloads of the spilled array.
...
// V6 paged scheduler walks (Q-block, chunk) tasks with eight adjacent
// causal queries sharing one physical KV traversal.
