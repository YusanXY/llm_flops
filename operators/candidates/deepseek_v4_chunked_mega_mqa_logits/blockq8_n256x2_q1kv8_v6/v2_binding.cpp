...
void launch_blockq8_n256x2_tmem1(
    int grid_size,
...
torch::Tensor v2_forward(
    ...
    TORCH_CHECK(q.dim() == 4, "q must have [B,8,H,D] layout");
    TORCH_CHECK(q.size(1) == 8 && q.size(2) == 64 && q.size(3) == 128,
                "V6 supports next_n=8, H=64, D=128");
    ...
    TORCH_CHECK(weights.dim() == 2 &&
                weights.size(0) == q.size(0) * 8 &&
                weights.size(1) == 64,
                "weights must have [B*8,64] layout");
    TORCH_CHECK(context_lens.dim() == 2 &&
                context_lens.size(0) == q.size(0) &&
                 context_lens.size(1) == 8,
                 "context_lens must have [B,8] layout");
    TORCH_CHECK(block_table.dim() == 2 &&
                block_table.size(0) == q.size(0),
                "block_table must have [B,max_pages] layout");
    ...
    const auto batch = static_cast<int>(q.size(0));
    const auto total_q = batch * kBlockQ;
    ...
    auto tensor_map_q = make_tma_2d(
        ...
        static_cast<uint64_t>(q.stride(2)),
        kHeadDim,
        4 * kHeads,
    ...
    const int grid_size = static_cast<int>(schedule_meta.size(0) - 1);
    auto stream = at::cuda::getCurrentCUDAStream(q.get_device()).stream();
    launch_blockq8_n256x2_tmem1(
        grid_size,
        total_q,
        padded_context,
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("forward", &v2_forward, "BLOCK_Q8/N256x2/TMEM1 SM100 paged MQA logits");
}
