...
CANDIDATE="$REPO/operators/candidates/deepseek_v4_chunked_mega_mqa_logits/single_request_stock_v0"
LOG="$CANDIDATE/full.log"
exec > >(tee "$LOG") 2>&1
...
flock -w7200 /tmp/mega-mqa-gpu3.lock bash -lc "
    ...
    ./bench.sh run \
      ...
      --candidate single_request_stock_v0 \
      --fail-fast \
      --timeout-s 3600
"
