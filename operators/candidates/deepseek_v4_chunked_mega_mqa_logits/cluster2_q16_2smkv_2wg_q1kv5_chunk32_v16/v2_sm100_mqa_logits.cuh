...
CUTLASS_DEVICE void sm100_mqa_logits_core_impl(const uint32_t logits_stride,
    ...
    DG_STATIC_ASSERT(kNumKVPeers == 2, "V16 requires a two-CTA cluster");
    DG_STATIC_ASSERT(kNumMathWarpGroups == 2 and kNumMathThreads == 256,
                     "V16 alternates four Q pairs across two consumer groups");
    DG_STATIC_ASSERT(SPLIT_KV == UMMA_M and kLocalSplitKV == 128,
                     "V16 requires a logical M256 split with M128 per CTA");
    DG_STATIC_ASSERT(BLOCK_Q == 8 and kNumQPairs == 4,
                     "V16 requires Q8 per CTA and four N256 passes");
    ...
    if (warp_idx == kSpecWarpStart) {
        ...
        cutlass::arch::warpgroup_reg_dealloc<kNumSpecializedRegisters>();
        DG_STATIC_ASSERT(not kIsFP4, "V16 currently targets the formal FP8 path");
...
// V16 pairs two Q8 CTAs.  The CTAs walk identical KV chunks and contribute
