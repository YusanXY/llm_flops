"""V9: V4 without an unnecessary grid-dependency wait.
...
* the kernel relies on ordinary same-stream ordering for schedule metadata and
  does not execute ``cudaGridDependencySynchronize`` without a PDL launch;
...
_BUILD = _HERE / ".build_blockq4_nogriddep_q1kv5_v9"
_BUILD.mkdir(exist_ok=True)

os.environ.setdefault("TORCH_CUDA_ARCH_LIST", "10.0a")

_ext = load(
    name="mega_chunked_mqa_blockq4_nogriddep_q1kv5_v9",
    ...
    with_cuda=True,
    verbose=os.environ.get("MEGA_V9_VERBOSE_BUILD", "0") == "1",
)
