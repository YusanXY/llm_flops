            ...
            const auto process_q_block = [&](auto num_valid_tokens_t) {
                ...
                for (uint32_t kv_split_idx = 0; kv_split_idx < num_kv_splits; ++ kv_split_idx) {
                    ...
                    for (uint32_t i = 0; i < kNumValidTokens; ++ i) {
                        // BLOCK_Q4 needs 4 x 64 FP32 weights.  Keeping that
                        ...
                        if constexpr (kIsReduceBF16) {
                            ...
                            for (uint32_t j = 0; j < kNumHeads; j += 4) {
                                ...
                                const uint32_t smem_addr = cute::cast_smem_ptr_to_uint(smem_weights_row + j);
                                asm volatile(
                                    "ld.shared.v4.b32 {%0, %1, %2, %3}, [%4];"
                                    : "=r"(w0), "=r"(w1), "=r"(w2), "=r"(w3)
                                    : "r"(smem_addr));

                                const auto a_0 = make_float2(accum[j], accum[j + 1]);
                                const auto a_1 = make_float2(fabsf(accum[j]), fabsf(accum[j + 1]));
                                const auto b_0 = make_float2(__uint_as_float(w0), __uint_as_float(w1));
                                sum_0 = __ffma2_rn(__fadd2_rn(a_0, a_1), b_0, sum_0);

                                const auto a_2 = make_float2(accum[j + 2], accum[j + 3]);
                                const auto a_3 = make_float2(fabsf(accum[j + 2]), fabsf(accum[j + 3]));
                                const auto b_1 = make_float2(__uint_as_float(w2), __uint_as_float(w3));
                                sum_1 = __ffma2_rn(__fadd2_rn(a_2, a_3), b_1, sum_1);
                            }
...
// V2 paged scheduler walks (Q-block, chunk) tasks with four adjacent
